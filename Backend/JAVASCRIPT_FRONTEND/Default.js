...
if (document.getElementById("ngmo") != undefined) {

    document.getElementById("ngmo").innerHTML = (getCookie("nm")) ? "◇" : "◆";

    document.getElementById("ngmo").addEventListener("click", () => {

        setCookie("nm", !getCookie("nm"), 30);
        document.getElementById("ngmo").innerHTML = (getCookie("nm")) ? "◇" : "◆";


        if (getCookie("nm") == "") {
            document.documentElement.style.setProperty('--CHANGEABLE-APP-BG', "rgb(27, 27, 37)")
            document.documentElement.style.setProperty("--NAVBAR-BOX-SHADOW", "0vmax -.5vmax 1vmax rgba(0,0,0, 1)")
            document.documentElement.style.setProperty("--CHANGEABLE-APP-TEXT", "rgb(255, 255, 255)")

        }else{
            document.documentElement.style.setProperty('--CHANGEABLE-APP-BG', "rgb(255, 255, 255)")
            document.documentElement.style.setProperty("--CHANGEABLE-APP-TEXT", "rgb(67, 63, 84)")
